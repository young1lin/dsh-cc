...
集成回归（`tests/integration/`，Python 标准库 unittest，不引入 JS 测试框架）对着真实链路活测：自动起停 3081 隔离实例、spawn 真 claude 进程走中转、Playwright 驱动真页面。密钥只在 `tests/integration/.env`（已 gitignore，本地留存；模板 `.env.example` 入库）；提交文件里不得出现本机绝对路径——路径一律由 `__file__` 推导或从凭据文件读取。改动任务栏/子代理/回退/转录渲染后必须跑对应用例类。
...
| `git-info.ts` | 会话 cwd 的实时 git 读数（分支 / worktree 名 / detached 短 sha）：一次 `rev-parse` + 短 TTL 缓存 + 在途去重，`GET /sessions/:id/git` 供状态条分支标签；SDK 控制通道没有分支查询，CLI 持久化的 `gitBranch` 只是滞后快照，所以直接问 git —— 冷会话也有 |
