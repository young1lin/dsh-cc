...
- **会话回退（/rewind 语义）**：任意历史消息 hover「回退」——对话回到这条消息**之前**，这条消息的文本自动填回输入框（可编辑后重发，与 CLI /rewind 的编辑重发一致）；可选同时回滚文件到该消息时（CLI 文件检查点，弹窗内预览将恢复的文件数与 ±行数，symlink 跳过数警示；需要会话进程在运行，冷会话自动禁用该选项）。实现上由「原生 fork 截断到锚点前一条 + 承袭会话设置 + 删除原会话」组合而成——SDK 没有原地截断 API，页面上读起来就是同一会话回到了过去；压缩后的转录渲染「对话已压缩 · 前 N tokens → 后 M tokens」分隔线（含 /compact 与自动压缩两种触发来源）
