...
- **复制回合**：任意用户消息 hover「复制回合」，把这一回合完整折成 Markdown 写入剪贴板——你的输入（含附图清单）、思考过程、正文、每次工具调用的参数与输出（`<system-reminder>` 脚手架已剥离、单条超长输出截断保护）、本地命令输出、错误与回合计时统计；回合仍在流式中时，未落定的最新内容一并带上并注明「仍在进行中」
